...
<head>
    ...
    <title>Balance+ | Admin Tips</title>
...
<body class="bg-bgBody h-screen flex overflow-hidden">
    ...
    <aside class="w-64 bg-white border-r border-slate-200 flex flex-col h-full shrink-0">
        ...
        <nav class="flex-1 px-4 space-y-1 overflow-y-auto mt-4">
            ...
            <a href="{{ route('admin.tips.index') }}" class="nav-active flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all">
  ...
  <main class="flex-1 overflow-y-auto p-10">
    <div class="max-w-6xl mx-auto text-center mb-10">
        <h2 id="currentTipTitle" class="text-3xl font-black text-slate-800 tracking-tight">All Health Tips</h2>
        <p class="text-slate-400 text-sm font-medium mt-1 italic">Expert advice for a healthier lifestyle</p>
    </div>
    ...
    <div class="flex justify-center gap-3 mb-8 flex-wrap">
        ...
        <button data-filter="nutrition" class="px-6 py-2 rounded-full text-xs font-bold transition-all text-slate-500 hover:bg-slate-50">NUTRITION</button>
        <button data-filter="fitness" class="px-6 py-2 rounded-full text-xs font-bold transition-all text-slate-500 hover:bg-slate-50">FITNESS</button>
        <button data-filter="wellness" class="px-6 py-2 rounded-full text-xs font-bold transition-all text-slate-500 hover:bg-slate-50">WELLNESS</button>
        <button data-filter="mental_health" class="px-6 py-2 rounded-full text-xs font-bold transition-all text-slate-500 hover:bg-slate-50">MENTAL HEALTH</button>
        <button data-filter="sleep" class="px-6 py-2 rounded-full text-xs font-bold transition-all text-slate-500 hover:bg-slate-50">SLEEP</button>
    </div>
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
        <table class="w-full text-left">
            <thead>
                <tr class="bg-slate-50/50 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">
                    <th class="px-8 py-6">Health Tip</th>
                    <th class="px-8 py-6">Category</th>
                    <th class="px-8 py-6">Content</th>
                    <th class="px-8 py-6 text-right">Action</th>
                </tr>
            </thead>
            <tbody>
                @forelse($tips as $tip)
                <tr class="hover:bg-slate-50/50 transition-colors group border-b border-slate-50 last:border-0 tip-row" data-category="{{ $tip->category }}">
                    <td class="px-8 py-5">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 rounded-2xl
                                @if($tip->category == 'nutrition') bg-green-50
                                @elseif($tip->category == 'fitness') bg-orange-50
                                @elseif($tip->category == 'wellness') bg-blue-50
                                @elseif($tip->category == 'mental_health') bg-purple-50
                                @elseif($tip->category == 'sleep') bg-indigo-50
                                @else bg-slate-50
                                @endif
                                flex items-center justify-center shadow-sm border border-white group-hover:scale-110 transition-transform">
                                <i class="fa-solid
                                    @if($tip->category == 'nutrition') fa-leaf text-green-500
                                    @elseif($tip->category == 'fitness') fa-dumbbell text-orange-500
                                    @elseif($tip->category == 'wellness') fa-droplet text-blue-500
                                    @elseif($tip->category == 'mental_health') fa-brain text-purple-500
                                    @elseif($tip->category == 'sleep') fa-moon text-indigo-500
                                    @else fa-lightbulb text-slate-500
                                    @endif
                                    text-lg"></i>
                            </div>
                            <div>
                                <h4 class="font-bold text-slate-900 text-sm">{{ $tip->title }}</h4>
                                <p class="text-[11px] text-slate-400 italic mt-0.5 capitalize">{{ str_replace('_', ' ', $tip->category) }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-8 py-5">
                        <span class="text-[9px] font-black text-primary uppercase bg-primary/5 px-2 py-0.5 rounded-md w-fit border border-primary/10 inline-block">
                            {{ str_replace('_', ' ', $tip->category) }}
                        </span>
                    </td>
                    <td class="px-8 py-5">
                        <div class="text-[11px] text-slate-500 leading-relaxed max-w-md">
                            {{ Str::limit($tip->content, 100) }}
                        </div>
                    </td>
                    <td class="px-8 py-5">
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('admin.tips.edit', $tip->id) }}" class="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-100 text-slate-400 hover:text-primary transition-all">
                                <i class="fa-solid fa-pen text-[10px]"></i>
                            </a>
                            <form action="{{ route('admin.tips.destroy', $tip->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this tip?')">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="4" class="px-8 py-12 text-center text-slate-400">
                        ...
                        <p class="font-semibold">No tips found</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
        ...
        <div class="flex justify-center mt-12 pb-10">
            <a href="{{ route('admin.tips.create') }}" class="bg-primary text-white px-12 py-4 rounded-[1.5rem] font-bold text-[12px] uppercase tracking-[0.15em] shadow-2xl shadow-primary/30 hover:-translate-y-1 active:scale-95 transition-all flex items-center gap-3">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('[data-filter]').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                const title = document.getElementById('currentTipTitle');
                ...
                // Update title
                const filterText = filter === 'all' ? 'All' : filter.replace('_', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
                title.innerText = filterText + " Health Tips";
